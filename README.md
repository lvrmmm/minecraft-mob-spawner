...
Добавлен подсчет созданных мобов - `MobSpawner` теперь отслеживает количество созданных объектов через фабрики.
Это позволяет анализировать работу системы и демонстрирует принцип инкапсуляции - клиентский код (`Main`) получает статистику через метод `getMobsCreated()`, не зная внутренней реализации подсчета.
...
### Версия 1.1: Добавление статистики
- **Добавлен счетчик** созданных мобов в классе `MobSpawner`
- **Удалены дополнительные мобы**, создаваемые напрямую (без фабрик)
- **Программа теперь чисто демонстрирует** работу паттерна "Абстрактная фабрика"
- **Статистика выводится** в конце выполнения
